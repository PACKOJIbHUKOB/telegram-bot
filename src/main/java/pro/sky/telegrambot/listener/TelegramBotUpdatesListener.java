    ...
    public int process(List<Update> updates) {
        updates.forEach(update -> {
            ...
            if (update.message().text().equals("/start")) {
                SendMessage message = new SendMessage(chatId, String.format("Привет, %s ! Введите задачу в формте: 01.01.0001 00:00 Введите Текст задачи: ... и я напомню тебе про нее", update.message().from().firstName()));
